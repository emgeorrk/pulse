> **pulse fork** of `fyne.io/systray` v1.12.2, wired in via a `replace`
> directive in the root `go.mod`. Local changes are marked `PATCH(pulse)`
> in `systray_darwin.m`: submenu parent items get their action cleared so
> that clicking them on macOS 14+ opens the submenu instead of dismissing
> the menu. The `example/` directory was dropped; everything else is
> untouched.
